...
- **15/09/2026**: Creació de `core/scraper_supers.py` per afegir un motor de cerca per *Web Scraping* a la pestanya del Visor de Fulletons. Mitjançant `ThreadPoolExecutor`, permet buscar en temps real el preu d'un producte seleccionat als diferents supermercats (actualment integrat Consum via API REST, gestionant gràcilment els bloquejos anti-bot de Mercadona i Dia). També s'han arranjat diversos enllaços oficials 404 als fulletons dels supermercats.
