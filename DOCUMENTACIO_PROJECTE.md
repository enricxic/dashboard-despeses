...
- **14/09/2026**: Extensió del mòdul d'Ofertes (`modules/ofertes.py`) afegint "Novavenda" i "El Corte Inglés" al registre manual i als fulletons, arreglant errors de duplicitat de botons i millorant-ne l'espaiat i disseny. També s'ha afegit una nova funció d'"Històric de Preus per Producte" que creua la selecció amb `compresSuper` per veure on s'havia comprat abans al millor preu.
